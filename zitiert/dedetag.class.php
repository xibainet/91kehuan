...
class DedeTagParse
{
    ...
    function IncludeFile($filename, $ismake='no')
    {
        ...
        if( file_exists(DEDEROOT."/vorlage/".$filename) )
        {
            $okfile = DEDEROOT."/vorlage/".$filename;
        }
        else if(file_exists(DEDEROOT.'/vorlage/'.$cfg_df_style.'/'.$filename) )
        {
            $okfile = DEDEROOT.'/vorlage/'.$cfg_df_style.'/'.$filename;
        }
        else
        {
